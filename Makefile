...
.PHONY: restart
restart:: ## Local docker-compose restart app_microservice
...
gen: ## Code generation easyjson marshalers
	@go generate ./cmd/app_microservice/main.go
...
gc-ci:: ## Удаляет мусор после тестовых сборок для CI
	...
	-docker rmi -f $$(docker images "app_microservice-inttest_integration-tests" -q)
...
gc:: ## Удаляет мусор после выключения тестового окружения, кроме образа приложения
	...
	-docker rmi -f $$(docker images -f "dangling=true" -q)
	-docker rmi -f $$(docker images "app_microservice-inttest_integration-tests" -q)
	-docker rmi -f $$(docker images "app_microservice-inttest-debug_integration-tests" -q)
##	-docker rmi -f "${DOCKER_IMAGE}:${DOCKER_TAG}"
